...
class Round():
    ...
    def generate_rounds(self, list_player):
        ...
        """for player in popped_players:
            print(popped_players)
            index = popped_players.index(player)
            players.append(popped_players.pop(index))"""
        return pairs
    @classmethod
    def generate_match(cls, list_player: list):
        ...
        match_object = Match(player0=list_player[first_player], player1=list_player[second_player])

        return (match_object, first_player, second_player)
    def is_same_pair(self, old_pairs, new_pair): #A renommer
        pass
    #TODO Retravaille la gen pour qu'a chaque appel de la fonction renvoi une liste de pairs ou les joueurs ne sont présent qu'une fois
    #TODO Ajout d'une logique pour que les joueurs soit différent a chaque fois dans mesure du possible    

class Match():
    def __init__(self, player0, player1, winner = None):
        self.player0 = player0
        self.player1 = player1
        self.winner = winner
    
    def __str__(self):
        #TODO A changer la phase de teste est finis
        return self.winner
    ...
    def to_dict(self):
        return{
            ...
            "winner":self.winner.ine if self.winner is not None else None
        }
